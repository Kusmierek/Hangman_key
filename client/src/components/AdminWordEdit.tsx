...
const AdminWordEdit = ({
  ...
  const categoriesOption = useMemo(() => {
    return categories.map((x: category) => {
      return (
  ...
  const onSubmit = async (data: wordAdmin, errors: unknown) => {
    console.log(data);
    await editWord(data._id, data.name, data.translation, data.category);
          ...
          <div className="px-6 py-6 lg:px-8">
            ...
            <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div>
                ...
                block w-full p-2.5"
                  {...register('category')}
                  defaultValue={modalData.category[0]}
                >
                  {categoriesOption}
                </select>
